# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  fish_abbreviation_tips = {
    pname = "fish_abbreviation_tips";
    version = "8ed76a62bb044ba4ad8e3e6832640178880df485";
    src = fetchFromGitHub {
      owner = "Gazorby";
      repo = "fish-abbreviation-tips";
      rev = "8ed76a62bb044ba4ad8e3e6832640178880df485";
      fetchSubmodules = false;
      sha256 = "sha256-F1t81VliD+v6WEWqj1c1ehFBXzqLyumx5vV46s/FZRU=";
    };
    date = "2023-01-23";
  };
  fish_async_prompt = {
    pname = "fish_async_prompt";
    version = "4c732cc043b8dd04e64a169ec6bbf3a9b394819f";
    src = fetchFromGitHub {
      owner = "acomagu";
      repo = "fish-async-prompt";
      rev = "4c732cc043b8dd04e64a169ec6bbf3a9b394819f";
      fetchSubmodules = false;
      sha256 = "sha256-YgqZINmY4nKphlqwHo2B0NfP4nmSxIIuAMUuoftI9Lg=";
    };
    date = "2023-02-26";
  };
  fish_autopairs = {
    pname = "fish_autopairs";
    version = "4d1752ff5b39819ab58d7337c69220342e9de0e2";
    src = fetchFromGitHub {
      owner = "jorgebucaran";
      repo = "autopair.fish";
      rev = "4d1752ff5b39819ab58d7337c69220342e9de0e2";
      fetchSubmodules = false;
      sha256 = "sha256-qt3t1iKRRNuiLWiVoiAYOu+9E7jsyECyIqZJ/oRIT1A=";
    };
    date = "2022-07-04";
  };
  fish_brew = {
    pname = "fish_brew";
    version = "328fc82e1c8e6fd5edc539de07e954230a9f2cef";
    src = fetchFromGitHub {
      owner = "oh-my-fish";
      repo = "plugin-brew";
      rev = "328fc82e1c8e6fd5edc539de07e954230a9f2cef";
      fetchSubmodules = false;
      sha256 = "sha256-ny82EAz0K4XYASEP/K8oxyhyumrITwC5lLRd+HScmNQ=";
    };
    date = "2023-05-11";
  };
  fish_done = {
    pname = "fish_done";
    version = "eb32ade85c0f2c68cbfcff3036756bbf27a4f366";
    src = fetchFromGitHub {
      owner = "franciscolourenco";
      repo = "done";
      rev = "eb32ade85c0f2c68cbfcff3036756bbf27a4f366";
      fetchSubmodules = false;
      sha256 = "sha256-DMIRKRAVOn7YEnuAtz4hIxrU93ULxNoQhW6juxCoh4o=";
    };
    date = "2024-04-11";
  };
  fish_jump = {
    pname = "fish_jump";
    version = "af285ff91fa9d0d0261b810e09f8a4a05a6b1307";
    src = fetchFromGitHub {
      owner = "oh-my-fish";
      repo = "plugin-jump";
      rev = "af285ff91fa9d0d0261b810e09f8a4a05a6b1307";
      fetchSubmodules = false;
      sha256 = "sha256-MVIXBKsfd7rrH7Dh7cksNI29YunqAGZvuZwdfrf1bZQ=";
    };
    date = "2021-02-14";
  };
  fish_puffer_fish = {
    pname = "fish_puffer_fish";
    version = "12d062eae0ad24f4ec20593be845ac30cd4b5923";
    src = fetchFromGitHub {
      owner = "nickeb96";
      repo = "puffer-fish";
      rev = "12d062eae0ad24f4ec20593be845ac30cd4b5923";
      fetchSubmodules = false;
      sha256 = "sha256-2niYj0NLfmVIQguuGTA7RrPIcorJEPkxhH6Dhcy+6Bk=";
    };
    date = "2024-03-03";
  };
  fish_sponge = {
    pname = "fish_sponge";
    version = "384299545104d5256648cee9d8b117aaa9a6d7be";
    src = fetchFromGitHub {
      owner = "meaningful-ooo";
      repo = "sponge";
      rev = "384299545104d5256648cee9d8b117aaa9a6d7be";
      fetchSubmodules = false;
      sha256 = "sha256-MdcZUDRtNJdiyo2l9o5ma7nAX84xEJbGFhAVhK+Zm1w=";
    };
    date = "2022-10-05";
  };
  fish_sudope = {
    pname = "fish_sudope";
    version = "83919a692bc1194aa322f3627c859fecace5f496";
    src = fetchFromGitHub {
      owner = "oh-my-fish";
      repo = "plugin-sudope";
      rev = "83919a692bc1194aa322f3627c859fecace5f496";
      fetchSubmodules = false;
      sha256 = "sha256-pD4rNuqg6TG22L9m8425CO2iqcYm8JaAEXIVa0H/v/U=";
    };
    date = "2021-04-11";
  };
  zimfw_completion = {
    pname = "zimfw_completion";
    version = "ef4d8a554ea8bb81e8fdb2d789289e8844d3a10f";
    src = fetchFromGitHub {
      owner = "zimfw";
      repo = "completion";
      rev = "ef4d8a554ea8bb81e8fdb2d789289e8844d3a10f";
      fetchSubmodules = false;
      sha256 = "sha256-5EAbnxu02UvRUG9MJurC1ccytKNf5PNKBvbV6L9PKlg=";
    };
    date = "2024-04-05";
  };
  zimfw_git = {
    pname = "zimfw_git";
    version = "de3a9f1553994d9fe5e6187f40ceac8afbfd1707";
    src = fetchFromGitHub {
      owner = "zimfw";
      repo = "git";
      rev = "de3a9f1553994d9fe5e6187f40ceac8afbfd1707";
      fetchSubmodules = false;
      sha256 = "sha256-mTmcp6hmtWc6012vaznbXdIojoRT5rux1HZ54VxtPY4=";
    };
    date = "2023-09-28";
  };
  zimfw_input = {
    pname = "zimfw_input";
    version = "b2a8a80071dc781dd1b075a5161d2a0225140916";
    src = fetchFromGitHub {
      owner = "zimfw";
      repo = "input";
      rev = "b2a8a80071dc781dd1b075a5161d2a0225140916";
      fetchSubmodules = false;
      sha256 = "sha256-alS02tiio5hfNviAdJ/uOhoNs4ESr7+dxLaJIY4Os3A=";
    };
    date = "2023-08-10";
  };
  zimfw_utility = {
    pname = "zimfw_utility";
    version = "6b74db5656509e5fe657901a9f0f52b6c7499904";
    src = fetchFromGitHub {
      owner = "zimfw";
      repo = "utility";
      rev = "6b74db5656509e5fe657901a9f0f52b6c7499904";
      fetchSubmodules = false;
      sha256 = "sha256-5p2SlB7OuycvDV7c/mCr5ddUw+npZfCD+LfMeyBeGtE=";
    };
    date = "2023-12-09";
  };
  zsh_completions = {
    pname = "zsh_completions";
    version = "efaa595ae93ce03836a1610dfd3364f081da8f3b";
    src = fetchFromGitHub {
      owner = "zsh-users";
      repo = "zsh-completions";
      rev = "efaa595ae93ce03836a1610dfd3364f081da8f3b";
      fetchSubmodules = false;
      sha256 = "sha256-9FIeMsI/RUeUwuyX9w8Hn91RLj14SL85BLPbAf32E0k=";
    };
    date = "2024-04-17";
  };
}
